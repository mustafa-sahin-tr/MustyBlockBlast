...
namespace MustyBlockBlast.Tests.EditMode
{
    ...
    public class BoardSystemHoldSlotTests
    {
        ...
        [Test]
        public void TryParkPiece_WithAnEmptyHold_EmptiesThatDockSlot()
        {
            FillDock(Single, HorizontalPair, VerticalTriple);

            bool held = _system.TryParkPiece(1);
        ...
        [Test]
        public void TryParkPiece_WithAnEmptyHold_ParksTheSamePieceAndColourUnchanged()
        {
            FillDock(Single, VerticalTriple, HorizontalPair);
            _trayModel.SetSlot(1, VerticalTriple, 7);

            _system.TryParkPiece(1);
        ...
        [Test]
        public void TryParkPiece_WithAnOccupiedHold_SwapsTheTwoPieces()
        {
            FillDock(Single, HorizontalPair, VerticalTriple);
            _system.TryParkPiece(0);

            bool swapped = _system.TryParkPiece(2);
        ...
        [Test]
        public void TryParkPiece_WithAnOccupiedHold_NeverLeavesTheDockHoldingBothPieces()
        {
            FillDock(Single, HorizontalPair, VerticalTriple);
            _system.TryParkPiece(0);
            int dockCountBefore = CountDockPieces();

            _system.TryParkPiece(2);
        ...
        [Test]
        public void TryParkPiece_DoesNotRefillTheTray()
        {
            FillDock(Single, HorizontalPair, VerticalTriple);
        ...
        [Test]
        public void TryParkPiece_PublishesNoPlacement()
        {
            FillDock(Single, HorizontalPair, VerticalTriple);
        ...
        [Test]
        public void TryParkPiece_LeavesTheScoreAndStreakUntouched()
        {
        ...
        [Test]
        public void TryParkPiece_WithAnOutOfRangeSlot_ChangesNothing()
        {
            FillDock(Single, HorizontalPair, VerticalTriple);

            Assert.IsFalse(_system.TryParkPiece(-1));
            Assert.IsFalse(_system.TryParkPiece(TrayModel.SLOT_COUNT));
            Assert.IsFalse(_trayModel.IsHoldOccupied);
        }

        [Test]
        public void TryParkPiece_OnAnAlreadyConsumedSlot_ChangesNothing()
        {
            _trayModel.SetSlot(0, Single, 1);
            _trayModel.SetSlot(1, HorizontalPair, 1);

            Assert.IsFalse(_system.TryParkPiece(2));
            Assert.IsFalse(_trayModel.IsHoldOccupied);
        }

        [Test]
        public void TryParkPiece_WithTheLastDockPieceAndAnEmptyHold_IsRefused()
        {
            // Parking it would leave nothing to drag and no way to refill — the dock is only ever
            // restocked as a placement's consequence.
            _trayModel.SetSlot(0, Single, 1);

            bool held = _system.TryParkPiece(0);
        ...
        [Test]
        public void TryParkPiece_WithTheLastDockPieceButAnOccupiedHold_IsAllowed()
        {
            FillDock(Single, HorizontalPair, VerticalTriple);
            _system.TryParkPiece(0);
            _trayModel.ConsumeSlot(1);

            bool held = _system.TryParkPiece(2);
        ...
        [Test]
        public void TryParkPiece_OnceTheRunIsOver_ChangesNothing()
        {
        ...
        public void HeldChanged_DoesNotFireForARefusedPark()
        {
            ...
            Assert.IsFalse(_system.TryParkPiece(0));
            Assert.IsFalse(_system.TryParkPiece(2));
        ...
        {
            // No inventory is wired into this fixture, so the parked piece is read the pre-charge way:
            // always swappable back out. A board that only it fits is therefore not a dead end.
        ...
        // --- Issue #202 (AC4): the parked piece is a move only while a charge can bring it back ---

        [Test]
        public void CheckGameOver_WithADeadDockAndAParkedPieceBehindAChargedPocket_DoesNotEndTheRun()
        {
            var powerUpModel = new PowerUpModel();
            powerUpModel.HoldCount.Value = 1;
            BoardSystem system = CreateSystemWithInventory(powerUpModel);
            ArrangeDeadDockWithOnlyTheParkedSingleFitting(system);

            bool placed = system.TryPlacePiece(0, new GridPosition(0, 0));

            Assert.IsTrue(placed);
            Assert.IsFalse(system.IsGameOver, "A charge in hand means the parked piece can still be played.");
            Assert.AreEqual(0, _gameOverBroker.Published.Count);
        }

        [Test]
        public void CheckGameOver_WithADeadDockAndAParkedPieceBehindAnEmptyPocket_EndsTheRun()
        {
            // The one reading the old rule got wrong once Hold had a charge: with none left, the swap
            // that returns the parked piece is refused, so the piece is stuck and the board only it
            // fits is a dead end after all. Leaving the run alive here would strand the player with
            // nothing they can actually do.
            var powerUpModel = new PowerUpModel();
            powerUpModel.HoldCount.Value = 0;
            BoardSystem system = CreateSystemWithInventory(powerUpModel);
            ArrangeDeadDockWithOnlyTheParkedSingleFitting(system);
        ...
        /// <summary>
        /// The same fixture as <see cref="CheckGameOver_WithADeadDockButAPlayableParkedPiece_DoesNotEndTheRun"/>
        /// but with an inventory wired in, so the game-over check has a Hold count to read.
        /// </summary>
        private BoardSystem CreateSystemWithInventory(PowerUpModel powerUpModel)
        {
        ...
        /// <summary>
        /// Parks the single, then leaves a dock of three-tall pieces on a board where no column has three
        /// free cells in a row — so after the next placement only the parked single fits anywhere.
        /// The park goes through the mechanism directly: the charge gate under test is the game-over
        /// check's, not the park's.
        /// </summary>
        private void ArrangeDeadDockWithOnlyTheParkedSingleFitting(BoardSystem system)
        {
            FillBoardLeavingTwoFreeCellsPerColumn();
            _trayModel.SetSlot(0, Single, 1);
            _trayModel.SetSlot(1, VerticalTriple, 2);
            _trayModel.SetSlot(2, VerticalTriple, 3);
            system.TryParkPiece(0);
            _trayModel.SetSlot(0, Single, 1);
        }
