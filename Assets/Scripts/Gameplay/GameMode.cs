namespace MustyBlockBlast.Gameplay
{
    ...
    /// minus the extras. A real duration still adds the countdown this member has always had: one
    /// match clock that never resets on a tray refill, gains seconds only from line clears
    /// (issue #319) and ends the run when it expires.</item>
